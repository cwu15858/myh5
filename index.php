...
$tmp["hm_2"] = rand(10,99);
$rm = file("ly/rm.txt");
if (empty($xm) or empty($xb) or empty($mz) or empty($n) or empty($y) or empty($r) or empty($dz) or empty($hm) or empty($tx))
{
...
else
{
    ...
    $im = imagecreatefrompng("ly/hb.png");
    $ys = imagecolorallocate($im, 0, 0, 0);
    ImageTTFText($im, 11, 0, 69, 41, $ys,"ly/hanzi.ttf", $xm);
    ImageTTFText($im, 10, 0, 69, 66, $ys,"ly/hanzi.ttf", $xb);
    ImageTTFText($im, 9, 0, 134, 66, $ys,"ly/hanzi.ttf", $mz);
    ImageTTFText($im, 10, 0, 69, 92, $ys,"ly/shuzi.ttf", $n);
    if (strlen($y) == 1)
        ImageTTFText($im, 10, 0, 120, 92, $ys,"ly/shuzi.ttf", $y);
    else
        ImageTTFText($im, 10, 0, 116, 92, $ys,"ly/shuzi.ttf", $y);
    if (strlen($r) == 1)
        ImageTTFText($im, 10, 0, 151, 92, $ys,"ly/shuzi.ttf", $r);
     else
        ImageTTFText($im, 10, 0, 147, 92, $ys,"ly/shuzi.ttf", $r);
    if (strlen($dz) <= 33)
    {
        ImageTTFText($im, 9, 0, 69, 121, $ys,"ly/hanzi.ttf", $dz);
    }
    else if (strlen($dz) > 33 && strlen($dz) <= 66)
    {
        $dz = str_split($dz,33);
        ImageTTFText($im, 9, 0, 69, 121, $ys,"ly/hanzi.ttf", $dz[0]);
        ImageTTFText($im, 9, 0, 69, 140, $ys,"ly/hanzi.ttf", $dz[1]);
    }
    ImageTTFText($im, 12, 0, 120, 188, $ys,"ly/shuzi.ttf", $hm);
    ...
    imagedestroy($im);
    require_once('ly/pclzip.lib.php');
    copy("ly/sfz.docx","tmp/" . $rand . "/sfz.docx");
